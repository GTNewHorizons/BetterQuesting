...
public class PanelTaskPartialRetrieval extends CanvasMinimum {

    private final TaskRetrieval task;
    private final IGuiRect initialRect;

    public PanelTaskPartialRetrieval(IGuiRect rect, TaskRetrieval task) {
        super(rect);
        this.task = task;
        initialRect = rect;
    }
    ...
    public void initPanel() {
        ...
        for (int i = 0; i < task.requiredItems.size(); i++) {
            ...
            GuiRectangle guiRectangle = new GuiRectangle(0, i * 48 + 16, 28, 28, 0);
            ...
            PanelTextBox text = new PanelTextBox(new GuiRectangle(32, i * 48 + 16, listW - 28, 28, 0), sb.toString());
            text.setColor(PresetColor.TEXT_MAIN.getColor());
            this.addPanel(text);

            if (task.requiredItems.size() - i == 1) continue;

            PanelTextBox testText = new PanelTextBox(new GuiRectangle(0, i * 48 + 50, 28, 10, 0), "OR");
