...
public class EventHandler {
    ...
    public void onClientChatReceived(ClientChatReceivedEvent event) {
        if (event.message != null) {
            ...
            if (index != -1) {
                ...
                IChatComponent translated =
                        new ChatComponentTranslation("betterquesting.msg.share_quest", questId, questName);
    ...
    public void onPlayerClone(Clone event) {
        ...
        QuestCache nCache =
                (QuestCache) event.entityPlayer.getExtendedProperties(QuestCache.LOC_QUEST_CACHE.toString());
    ...
    public void onLivingUpdate(BQLivingUpdateEvent event) {
        ...
        if (refreshCache
                || player.ticksExisted % 200 == 0) // Refresh the cache if something changed or every 10 seconds
        {
    ...
    public void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (BetterQuesting.proxy.isClient()
                ...
                && MinecraftServer.getServer()
                        .getServerOwner()
                        .equals(event.player.getGameProfile().getName())) {
        ...
        if (QuestSettings.INSTANCE.getProperty(NativeProps.HARDCORE)
            ...
            if (lives <= 0) {
                ...
                if (server.isSinglePlayer() && mpPlayer.getCommandSenderName().equals(server.getServerOwner())) {
                    mpPlayer.playerNetServerHandler.kickPlayerFromServer(
                    ...
                    UserListBansEntry userlistbansentry = new UserListBansEntry(
                            mpPlayer.getGameProfile(), null, "(You just lost the game)", null, "Death in Hardcore");
    ...
    public void onDataUpdated(DatabaseEvent.Update event) {
        ...
        if (screen instanceof INeedsRefresh)
            Minecraft.getMinecraft().func_152343_a(Executors.callable(((INeedsRefresh) screen)::refreshGui));
    }
    ...
    public void onCommand(CommandEvent event) {
        ...
        if (server != null
                && (event.command.getCommandName().equalsIgnoreCase("op")
                        || event.command.getCommandName().equalsIgnoreCase("deop"))) {
            ...
            if (playerMP != null && NameCache.INSTANCE.updateName(playerMP)) {
                ...
                if (party != null) {
                    ...
                    NetNameSync.sendNames(
                            new EntityPlayerMP[] {playerMP}, new UUID[] {QuestingAPI.getQuestingUUID(playerMP)}, null);
                }
            }
        }
