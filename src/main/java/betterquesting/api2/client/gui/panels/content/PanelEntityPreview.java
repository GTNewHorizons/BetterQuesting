...
public class PanelEntityPreview implements IGuiPanel
{
	...
	public void drawPanel(int mx, int my, float partialTick)
    {
		...
		if (EntityList.getEntityString(entity).contains("Wisp")) {
			...
			RenderUtils.RenderEntity(bounds.getX() + sizeX/2, (int) (bounds.getY() + sizeY/2 + head.height*.25 * scale), (int)scale, yawDriver.readValue(), pitch, head);
			RenderUtils.RenderEntity(bounds.getX() + sizeX/2, (int) (bounds.getY() + sizeY/2 + head.height*1.5 * scale), (int)scale, yawDriver.readValue(), pitch, head);
		} else
			pitch = pitchDriver.readValue();
